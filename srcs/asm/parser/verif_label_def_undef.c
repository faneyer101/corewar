...
/*   Updated: 2020/02/28 09:46:57 by faneyer          ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
int		search_label_undefine(t_list_label *begin_undefine, t_token *data)
{
	...
	while (undefine)
	{
		if (ft_strcmp(undefine->name, data->data) == 0 && ft_strlen(undefine->name) == ft_strlen(data->data))
			return (TRUE);
...
int		search_label_define(t_list_label *begin_define, t_token *data)
{
	...
	while (define)
	{
		if (ft_strcmp(define->name, data->data) == 0 && ft_strlen(define->name) == ft_strlen(data->data))
			return (TRUE);
		define = define->dnext;
	}
	return (FALSE);
