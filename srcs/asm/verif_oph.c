...
/*   Updated: 2020/02/28 16:20:00 by faneyer          ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (delimiter(COMMENT_CMD_STRING[0], "d#Ns*") ||
		delimiter(NAME_CMD_STRING[0], "d#Ns*") ||
		delimiter(SEPARATOR_CHAR, "#Ns*") ||
		delimiter(DIRECT_CHAR, "#Ns*") || delimiter(LABEL_CHAR, "#Ns*") ||
		delimiter(ALT_COMMENT_CHAR, "dNs*") ||
		delimiter(COMMENT_CHAR, "dNs*") ||
		delimiter(SEPARATOR_CHAR, "nc") ||
		delimiter(DIRECT_CHAR, "nc") || delimiter(LABEL_CHAR, "nc") ||
		delimiter(ALT_COMMENT_CHAR, "nc") ||
		delimiter(COMMENT_CHAR, "nc") ||
		COMMENT_CHAR == ALT_COMMENT_CHAR || LABEL_CHAR == DIRECT_CHAR ||
		LABEL_CHAR == SEPARATOR_CHAR || LABEL_CHAR == DNEGATIF ||
		DIRECT_CHAR == SEPARATOR_CHAR || DIRECT_CHAR == DNEGATIF ||
		SEPARATOR_CHAR == DNEGATIF)
...
static void	verif_cmd(t_asm *master, int i, int j)
{
	...
	while (++i < 17)
	{
		...
		while (++j < 17)
		{
			if (i != j && !ft_strcmp(master->tab_op[i].name,
			master->tab_op[j].name))
...
void		verif_error_first(t_asm *master, char *file)
{
	...
	if (ft_strcmp(NAME_CMD_STRING, COMMENT_CMD_STRING) == 0)
		...
		LABEL_CHARS[0] == '\0' || ALT_COMMENT_CHAR == '\0' ||
		COMMENT_CHAR == '\0')
		print_error("op.h don't have params empty\n");
	else if (cheak_label_chars())
		print_error("op.h LABEL_CHARS juste alpha and/or _ and/or digit.\n");
